...
import SidePot from "./SidePot.js";
...
class Table {
  ...
  endWithoutShowdown () {
    const winner = this.unfoldPlayers()[0];
    winner && winner.winHand(this.pot);
    winner && this.winMessages.push(
  ...
  dealNextStreet(){
    ...
    if(length === 0){
        ...
        this.determineSidePotWinner();
        this.determineMainPotWinner();
    }
  }
  determineSidePotWinner(){
    ...
    determineWinner(amount, seats){
        ...
        const findHandOwner = (cards) => {
            ...
            lodsh.isEqual(participant.solverCards.sort(),cards),
            );
    ...
    handleRaise(socketId, amount){
        ...
        if(seat){
            ...
            return{
                seatId: seat.id,
                message: `${seat.player.name} raise to $${amount.toFixed(2)}`,
            };
        }else{
            return null;
        }
